...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "create":
        await query.edit_message_text(
            ...
            "Временно в разработке. Скоро будет доступно!\n\n"
            "Пока можете использовать команду /new",
            parse_mode='Markdown'
        )
    elif query.data == "list":
        await query.edit_message_text(
            "📋 *Ваши напоминания*\n\n"
            "Раздел в разработке. Скоро будет доступно!",
            parse_mode='Markdown'
        )
        ...
        await help_command_with_update(query)

async def help_command_with_update(query):
    """Помощь через кнопку"""
    await query.edit_message_text(
        "📚 *Помощь по боту:*\n\n"
        "Сейчас доступно:\n"
    ...
    if update and update.effective_message:
        try:
            await update.effective_message.reply_text(
                "⚠️ Произошла ошибка. Попробуйте еще раз позже."
            )
        except:
            pass
